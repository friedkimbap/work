...
//                        receiveMessage();
    ...
    public void sendFile(){
        String filename = t_input.getText();
        if(filename.isEmpty()) return;

        File file = new File(filename);
        if(!file.exists()){
            printDisplay("파일이 존재하지 않습니다.");
            return;
        }
        ...
        try {
            ...
            out.close(); // 한 번에 하나의 파일만 전송 ㄱㄴ
        } catch (FileNotFoundException e) {
            printDisplay(">> 파일이 존재하지 않습니다.");
            return;
        } catch (UnsupportedEncodingException e){
            printDisplay(">> 인코딩 형식이 이상합니다");
            return;
        } catch (IOException e){
            printDisplay(">> 파일을 읽을 수 없습니다.");
            return;
        }
    }
